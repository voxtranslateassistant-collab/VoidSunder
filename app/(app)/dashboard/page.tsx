...
function relativeTime(value: string | null) {
  if (!value) return "Sem execuções concluídas";
...
export default async function DashboardPage() {
  ...
  return <><TopBar title="Painel" subtitle="Postura operacional em tempo real" action={<Link href="/scans/new" className="inline-flex h-8 items-center gap-2 bg-bone-white px-3 text-xs font-medium text-pure-black"><Radar className="size-4" />Novo Scan</Link>} />
    <main className="tactical-grid flex-1 overflow-y-auto"><div className="mx-auto max-w-[1600px] space-y-6 p-8">
      ...
      <Card><CardHeader><div><CardTitle className="flex items-center gap-2"><Clock3 className="size-4 text-prism-cyan" />Estado da operação</CardTitle><p className="mt-1 text-xs text-graphite-veil">Leitura baseada nos jobs persistidos; não substitui o monitoramento da infraestrutura do worker.</p></div><StatusDot status={overview.active > 0 ? "running" : "completed"} /></CardHeader><CardContent className="flex flex-wrap gap-x-8 gap-y-2 text-sm"><p><span className="text-graphite-veil">Última conclusão:</span> <span className="text-bone-white">{relativeTime(overview.latestCompletedAt)}</span></p><p><span className="text-graphite-veil">Fila ativa:</span> <span className="text-bone-white">{overview.active} job{overview.active === 1 ? "" : "s"}</span>{overview.oldestPendingAt && <> <span className="text-graphite-veil">desde {relativeTime(overview.oldestPendingAt)}</span></>}</p></CardContent></Card>
