...
export default async function FindingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const finding = await getOperationalFindingById(id);
  if (!finding) notFound();
  const evidence = await getOperationalEvidenceByFinding(finding.id);
  const meta = [
    ["Alvo", finding.assetName], ["Endpoint", finding.endpoint], ["Verificação", ENGINE_LABEL[finding.engine]], ["CWE", finding.cwe ?? "—"], ["Categoria OWASP", finding.owaspCategory ?? "—"], ["CVSS", finding.cvss?.toFixed(1) ?? "—"], ["Origem", "Job operacional"], ["Detectado", formatRelativeTime(finding.detectedAt)],
  ];
  return <><TopBar title={finding.title} subtitle={`${finding.assetName} · ${shortUrl(finding.endpoint, 60)}`} />
