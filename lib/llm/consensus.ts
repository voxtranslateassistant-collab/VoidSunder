...
type ProviderResult = { provider: LlmProviderId; model: string; status: "completed" | "failed"; confidence: number; analysis: string; error: string | null; latencyMs: number };

function evidenceDigest(findings: Finding[]) {
  return findings.slice(0, 30).map((f) => `- [${f.validationStatus}] ${f.title}; severidade=${f.severity}; evidência=${(f.evidenceMasked ?? f.summary).slice(0, 280)}; correção=${(f.recommendedFix ?? f.remediation).slice(0, 180)}`).join("\n");
}
...
export async function orchestrateConsensus(input: { jobId: string; scanId?: string; findings: Finding[] }) {
  ...
  const results = await Promise.all(providers.map(async (provider): Promise<ProviderResult> => {
    ...
    return { provider, model: PROVIDERS[provider].model, status: result.ok ? "completed" : "failed", confidence: result.ok ? confidence(result.text) : 0, analysis: result.text.slice(0, 8000), error: result.error, latencyMs: result.latencyMs };
  }));
  ...
  const consensus = { id: randomUUID(), state: completed.length >= 2 ? "cross_validated" : "single_provider", confidence: Number(average.toFixed(2)), providersUsed: completed.map((result) => result.provider), divergences, report: completed.map((result) => `## ${PROVIDERS[result.provider].label}\n${result.analysis}`).join("\n\n") };
  const { data: run, error } = await supabase.from("ai_consensus_runs").insert({ id: consensus.id, org_id: membership.org_id, scan_id: input.scanId ?? null, job_id: input.jobId, providers: providers, consensus, created_by: user.user.id }).select("id").single();
