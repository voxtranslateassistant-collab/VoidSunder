  ...
  return <div className="grid gap-6 lg:grid-cols-3">
    <div className="lg:col-span-2"><Card><CardHeader><CardTitle>Criar job de validação</CardTitle></CardHeader><CardContent>
      <form onSubmit={submit} className="space-y-5">
        ...
        {profile === "api_validation" && <div className="flex flex-col gap-1.5"><label htmlFor="openapi-spec" className="text-micro-caps text-graphite-veil">OpenAPI em JSON (opcional)</label><textarea id="openapi-spec" value={openapiSpec} onChange={(event) => setOpenapiSpec(event.target.value)} placeholder='{"openapi":"3.0.0","paths":{"/health":{"get":{}}}}' spellCheck={false} className="min-h-36 w-full border border-ash-border bg-surface-2 p-3 font-mono text-xs text-bone-white outline-none transition-colors placeholder:text-graphite-veil focus:border-prism-cyan" /><p className="text-xs text-graphite-veil">O worker apenas mapeia rotas e métodos declarados. Não envia requisições autenticadas nem executa ações da API.</p></div>}
